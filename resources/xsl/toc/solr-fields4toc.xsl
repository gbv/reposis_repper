...
<xsl:stylesheet version="1.0"
  ...
  <xsl:template match="mods:relatedItem[contains('host series',@type)]" mode="toc">
    <xsl:if test="mods:relatedItem[contains('host series',@type)]">
      <xsl:variable name="grandma" select="document(concat('mcrobject:', ./mods:relatedItem[contains('host series',@type)]/@xlink:href))/mycoreobject" />
      <xsl:if test="$grandma//servflags[@class='MCRMetaLangText']/servflag[@type='tocLayout']">
